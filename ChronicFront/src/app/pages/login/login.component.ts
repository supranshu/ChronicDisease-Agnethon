...
export class LoginComponent implements OnInit {
  ...
  public doLoginU() {
    ...
    this.http
      .get<any>(
        `${baseUrl}/user-login/${this.loginData.email}/${this.loginData.password}`
      )
      .subscribe(
        (response) => {
  ...
  public doLoginD() {
    this.http
      .get<any>(
        `${baseUrl}/doctor-login/${this.loginData.email}/${this.loginData.password}`
      )
      .subscribe(
        (response) => {
          ...
          this.snack.open('Login successful!!!', '', { duration: 3000 });
          localStorage.setItem('email', response.email);
          localStorage.setItem('docName', response.docName);
